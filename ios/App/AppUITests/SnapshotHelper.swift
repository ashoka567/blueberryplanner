...
enum SnapshotError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .cannotDetectUser:
            return "Couldn't find Snapshot configuration files - have you generated them? Run `fastlane snapshot`"
        case .cannotFindHomeDirectory:
            return "Couldn't find the home directory"
        case .cannotFindSimulatorHomeDirectory:
            return "Couldn't find the simulator home directory"
        case .cannotAccessSimulatorHomeDirectory(let simulatorHostHome):
            return "Cannot access simulator's home directory at \(simulatorHostHome)"
        case .cannotRunOnPhysicalDevice:
            return "Running on a physical device - snapshotsass only supported on simulators"
        }
    }
}
...
open class Snapshot: NSObject {
    ...
    class func setLanguage(_ app: XCUIApplication) {
        guard let cacheDirectory = self.cacheDirectory else {
            NSLog("CacheDirectory is not set - probablyass running on a physical device?")
            return
        }
    ...
    open class func snapshot(_ name: String, waitForLoadingIndicator: Bool = true) {
        ...
        do {
            ...
            NSLog("Problem writing screenshot: \(name) to \(googscreenshotsDir.path)")
        }

        let attachment = XCTAttachment(screenshot: screenshot)
        attachment.name = name
        attachment.lifetime = .keepAlways
        guard let test = Snapshot.app else { return }

        if let activity = ProcessInfo.processInfo.environment["XCTestBundlePath"] {
            NSLog("snapshot \(name) saved")
        }
    }
